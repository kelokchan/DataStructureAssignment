...
public:
	...
	void setValue(int value, int inputIndex) {
		...
		if (size == 0 || head->index > inputIndex) {
			...
			while (current != NULL) {
				if (current->index == inputIndex) {
				...
				else if (current->index < inputIndex) {					//current is smaller than input
					if (current->link != NULL) {						//check next
						if (current->link->index > inputIndex) {		//next index is bigger than input
							...
							current = current->link;					//next index is smaller than input, current shifts to current link
						}
					}
					else {
						insertAtEnd(value, inputIndex);					//current link is null, insert after current
						break;
					}
				}
			}
	...
	void insertValueAfter(int value, int inputIndex, NodeType * prev) {			//insert newNode after prev
		NodeType * newNode = new NodeType;
		newNode->info = value;
		newNode->index = inputIndex;
	...
	void sumNodes(LinkedList &list2) {
		...
		NodeType * prev = NULL;
		while (other != NULL) {
			if (current == NULL || other->index > tail->index) {					//list is 0 or tail index < other index
				...
				if (current->index == other->index) {							//same index, update current value
					...
					this->insertValueAfter(other->info, other->index, prev);				//insert other after current
					prev = prev->link;												//prev is now the inserted node
				}
			}
			other = other->link;
		}
...
SM::SM(int rows, int columns)
{
	...
	cout << "Creating a sparse matrix of dimensions " << this->rows << "-" << this->columns << endl;
}
...
int main2()
{
	cout << "Enter sparse matrix 1 dimension in following format: rowIndex columnIndex " << endl;
	int rows, columns;
	cin >> rows;
	cin >> columns;
	...
	cout << "Enter sparse matrix 2 dimension in following format: rowIndex columnIndex " << endl;
	cin >> rows;
	cin >> columns;
	SM sm2(rows, columns);
